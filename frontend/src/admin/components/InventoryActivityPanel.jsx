...
export default function InventoryActivityPanel({ items }) {
  return (
    <div className="rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
      <div className="flex items-center justify-between gap-3">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.16em] text-slate-400">Inventory activity</p>
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Recent admin actions on stock, pricing, and warehouse control.</p>
        </div>
      ...
      <div className="mt-4 space-y-3">
        {items.map((item) => (
          <div key={item.id} className="rounded-2xl border border-slate-200 p-3 dark:border-slate-800">
            ...
            <p className="mt-2 text-xs uppercase tracking-[0.14em] text-slate-400">{item.actor} · {formatInventoryDate(item.timestamp)}</p>
          </div>
        ))}
      </div>
    </div>
