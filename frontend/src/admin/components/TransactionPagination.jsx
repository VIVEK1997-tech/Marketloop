...
import { rowsPerPageOptions } from '../transactions/transactionUtils.js';

export default function TransactionPagination({ start, end, total, rowsPerPage, currentPage, totalPages, onRowsPerPageChange, onPageChange }) {
  const pages = [];
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-4 rounded-[1.5rem] border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
      <p className="text-sm text-slate-500 dark:text-slate-400">Showing {start}-{end} of {total.toLocaleString('en-IN')} transactions</p>
